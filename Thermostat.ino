...
#define TEMP_PIN 2
#define RELAY_PIN 4
#define HEATING_LED_PIN 5
#define OVERRIDE_LED_PIN 6

#define SW_SET_PIN 8
#define SW_UP_PIN 9
#define SW_DOWN_PIN 10
#define SW_START_PIN 11
#define SW_CANCEL_PIN 12
...
void setup()
{
  ...
  EEPROM.get(SETBACK_TEMP_ADDR, setback_temp);
  EEPROM.get(OVERRIDE_TEMP_ADDR, override_temp);
  EEPROM.get(OVERRIDE_MINUTES_ADDR, override_minutes);
  EEPROM.get(CYCLE_LIMIT_ADDR, cycle_limit);
  EEPROM.get(CYCLES_PER_HOUR_ADDR, cycles_per_hour);
  EEPROM.get(ANTICIPATION_ADDR, anticipation);
...
void UpdateEEPROM() {
  ...
  EEPROM.get(SETBACK_TEMP_ADDR, old_setback_temp);
  EEPROM.get(OVERRIDE_TEMP_ADDR, old_override_temp);
  EEPROM.get(OVERRIDE_MINUTES_ADDR, old_override_minutes);
  EEPROM.get(CYCLE_LIMIT_ADDR, old_cycle_limit);
  EEPROM.get(CYCLES_PER_HOUR_ADDR, old_cycles_per_hour);
  EEPROM.get(ANTICIPATION_ADDR, old_anticipation);
  
  if (setback_temp != old_setback_temp)         EEPROM.put(SETBACK_TEMP_ADDR, setback_temp);
  if (override_temp != old_override_temp)       EEPROM.put(OVERRIDE_TEMP_ADDR, override_temp);
  if (override_minutes != old_override_minutes) EEPROM.put(OVERRIDE_MINUTES_ADDR, override_minutes);
  if (cycle_limit != old_cycle_limit)           EEPROM.put(CYCLE_LIMIT_ADDR, cycle_limit);
  if (cycles_per_hour != old_cycles_per_hour)   EEPROM.put(CYCLES_PER_HOUR_ADDR, cycles_per_hour);
  if (anticipation != old_anticipation)         EEPROM.put(ANTICIPATION_ADDR, anticipation);
}
